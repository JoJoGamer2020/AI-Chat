...
class MainActivity : AppCompatActivity() {
    ...
    private fun getDefaultModels(): Pair<Array<String>, Array<String>> {
        val modelIds = arrayOf(
            ...
            "deepseek", "gemini-search", "claude-fast", "perplexity-fast", 
            "perplexity-reasoning", "kimi", "nova-fast", "glm", "minimax", "polly", 
            "qwen-safety", "custom"
        )
    ...
    private fun syncModelsFromAPI() {
        Toast.makeText(this, "Syncing models...", Toast.LENGTH_SHORT).show()
                    ...
                    if (response.isSuccessful) {
                        ...
                        for (i in 0 until jsonArray.length()) {
                            ...
                            for (j in 0 until inputMods.length()) {
                                if (inputMods.getString(j) == "text") hasTextInput = true
                            }
                            for (j in 0 until outputMods.length()) {
                                if (outputMods.getString(j) == "text") hasTextOutput = true
                            }
                            
                            if (hasTextInput && hasTextOutput && !paidOnly) {
                                val filtered = JSONObject().apply {
                        ...
                        withContext(Dispatchers.Main) {
                            Toast.makeText(this@MainActivity, "Synced ${filteredModels.length()} models", Toast.LENGTH_SHORT).show()
